...
}:
{
  ...
  options = {
    appflowy_db.enable = lib.mkEnableOption "enables an appflowy server running in a docker container.";
  };

  config = lib.mkIf config.appflowy_db.enable {
    # Runtime
    virtualisation.docker = {
      enable = true;
      autoPrune.enable = true;
    };
    virtualisation.oci-containers.backend = "docker"; #TODO: Convert this container to podman, Nix will throw errors otherwise.
